...
- Tombol ini berjalan pada versi web app yang **ter-deploy**. Bila deployment masih lama,
  jalankan **`sinkronSekarang`** dari editor (dropdown fungsi → Run) — dari editor selalu
  kode terbaru yang dipakai, jadi sinkron tidak pernah terkunci oleh deployment lama.
...
| Fungsi | Cakupan |
|---|---|
| `jalankanSemuaTest` | **241 test unit domain** — murni, tidak menyentuh data nyata. Hasilnya mencetak **penanda versi** (`VERSI_APP`, saat ini `2026.08.29-g`); bila jumlah test atau versinya tidak cocok dengan baris ini, ada file yang belum tersalin ke Apps Script. |
